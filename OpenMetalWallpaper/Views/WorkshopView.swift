...
struct WorkshopView: View {
    ...
    let sorts = [("热门", "trend"), ("最新", "mostrecent"), ("订阅", "total_subscriptions")]
    ...
    var body: some View {
            ...
            VStack(spacing: 12) {
                HStack(spacing: 12) {
                    HStack {
                        ...
                        TextField("搜索壁纸...", text: $searchText)
                            ...
                            .onSubmit { loadItems(forceRefresh: true) }
                        if !searchText.isEmpty {
                            Button(action: { searchText = ""; loadItems(forceRefresh: true) }) {
                ...
                HStack {
                    ...
                    HStack(spacing: 4) {
                        ...
                        Text("排序:").font(.caption).foregroundColor(.secondary).padding(.leading, 8)
            ...
            ZStack {
                if items.isEmpty && !isLoading {
                    VStack {
                        ...
                        Button("刷新") { loadItems(forceRefresh: true) }
                    }
                    .transition(.opacity)
                } else {
                    ScrollView {
                        ...
                        HStack {
                            Button("上一页") { if currentPage > 1 { currentPage -= 1; loadItems(forceRefresh: false) } }.disabled(currentPage <= 1)
                            Text("\(currentPage)").monospacedDigit().padding(.horizontal)
                            Button("下一页") { currentPage += 1; loadItems(forceRefresh: false) }
                        }.padding(.bottom)
                    }
        ...
        .onChange(of: selectedType) { loadItems(forceRefresh: true) }
        .onChange(of: selectedGenre) { loadItems(forceRefresh: true) }
        .onChange(of: selectedSort) { loadItems(forceRefresh: true) }
        .onAppear {
            if items.isEmpty { loadItems(forceRefresh: false) }
        }
    }
    
    private func loadItems(forceRefresh: Bool) {
        if forceRefresh { currentPage = 1 }
        withAnimation { isLoading = true }
        steam.fetchWorkshopItems(page: currentPage, searchText: searchText, type: selectedType, genre: selectedGenre, sort: selectedSort, forceRefresh: forceRefresh) { newItems in
