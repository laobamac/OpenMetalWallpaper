...
class WebPlayerEngine: NSObject, WallpaperPlayer, WKNavigationDelegate, WKScriptMessageHandler {
    ...
    func load(url: URL, options: WallpaperOptions) {
        ...
        // MARK: - Smart Bridge & Input Injection Script
        let bridgeScript = """
        ...
        // 全局事件注入接口 (Global Mouse Injection Interface)
        window.__omw_injectMouse = function(type, x, y) {
    ...
    private func sendInitialState() {
        ...
        setInteractive(options.isInteractive) // Apply initial interactive state
    }
    ...
    // MARK: - [New] Mouse Event Injection
    func injectMouseEvent(type: String, x: CGFloat, y: CGFloat) {
        // 将坐标转换为 webView 内部坐标
        // WebView 坐标系 Y 轴向下，Global Mouse 坐标 Y 轴向上，传入前需外部转换
        let js = "if(window.__omw_injectMouse) { window.__omw_injectMouse('\(type)', \(x), \(y)); }"
        evaluateJS(js)
    }
    
    // MARK: - [New] CSS Level Interaction Lock
    func setInteractive(_ allowed: Bool) {
        // "auto" allows interaction, "none" passes clicks through (and effectively disables web interaction)
        let val = allowed ? "auto" : "none"
        // Also set user-select to prevent text selection if needed, though pointer-events usually handles clicks
        let js = "document.body.style.pointerEvents = '\(val)'; document.body.style.userSelect = '\(val)';"
        evaluateJS(js)
    }
}
