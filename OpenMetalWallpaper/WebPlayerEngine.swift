/*
 ...
 Description: Web Engine with CSS Rotation support.
*/
...
class LocalSchemeHandler: NSObject, WKURLSchemeHandler {
    ...
    func webView(_ webView: WKWebView, start urlSchemeTask: WKURLSchemeTask) {
        ...
        guard let decodedPath = pathString.removingPercentEncoding else { urlSchemeTask.didFailWithError(URLError(.badURL)); return }
        let localURL = rootFolder.appendingPathComponent(decodedPath)
        do {
            let data = try Data(contentsOf: localURL)
            let ext = localURL.pathExtension.lowercased()
            let mime = self.mimeType(for: ext)
            let response = HTTPURLResponse(url: url, statusCode: 200, httpVersion: "HTTP/1.1", headerFields: ["Content-Type": mime, "Access-Control-Allow-Origin": "*", "Cache-Control": "no-cache"])!
            urlSchemeTask.didReceive(response); urlSchemeTask.didReceive(data); urlSchemeTask.didFinish()
        } catch {
            let response = HTTPURLResponse(url: url, statusCode: 404, httpVersion: nil, headerFields: nil)!
            urlSchemeTask.didReceive(response); urlSchemeTask.didFinish()
        }
    }
    ...
    private func mimeType(for `extension`: String) -> String {
        ...
        case "fnt": return "text/plain"
        default: if let type = UTType(filenameExtension: `extension`)?.preferredMIMEType { return type }; return "application/octet-stream"
        }
    }
}
...
class WebPlayerEngine: NSObject, WallpaperPlayer, WKNavigationDelegate, WKUIDelegate {
    ...
    func pause() {
        ...
        wv.takeSnapshot(with: config) { [weak self] image, error in
            ...
            overlay.image = snapshot; overlay.imageScaling = .scaleAxesIndependently; overlay.autoresizingMask = [.width, .height]
            self.containerView?.addSubview(overlay)
            self.pauseOverlay = overlay
            self.webView?.stopLoading(); self.webView?.removeFromSuperview(); self.webView = nil; self.schemeHandler = nil
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        ...
        if let overlay = self.pauseOverlay {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                overlay.removeFromSuperview(); self.pauseOverlay = nil
    ...
    private func saveTempImage(_ image: NSImage, for url: URL) {
        if let tiff = image.tiffRepresentation, let bitmap = NSBitmapImageRep(data: tiff), let pngData = bitmap.representation(using: .png, properties: [:]) { try? pngData.write(to: getTempPath(for: url)) }
    }
    private func cleanupTempImage(for url: URL) { try? FileManager.default.removeItem(at: getTempPath(for: url)) }
    
    func stop() {
        webView?.stopLoading(); webView?.configuration.userContentController.removeAllUserScripts(); webView?.removeFromSuperview(); webView = nil; schemeHandler = nil
    ...
    func setBackgroundColor(_ color: NSColor) {
        ...
        let r = String(format: "%.3f", color.redComponent); let g = String(format: "%.3f", color.greenComponent); let b = String(format: "%.3f", color.blueComponent)
